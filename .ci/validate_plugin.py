...
# spec defines no root-level plugin.json; assert none was scaffolded.
if (ROOT / "plugin.json").exists():
    err("root plugin.json must not exist — the spec defines none; the manifest is .claude-plugin/plugin.json")
